...
const DEFAULT_LEAD = {
  name:'',category:'',subcategory:'',country:'Danmark',
...
export default function CRMApp() {
  ...
  const saveLead = async () => {
    ...
    try {
      ...
      const payload = {
        name: editLead.name, category: editLead.category, subcategory: editLead.subcategory||'', country: editLead.country,
  ...
  const importLeads = async () => {
    ...
    try {
      for(const lead of iPrev){
        const {_outreaches,_newCountry,subcategory,...leadData} = lead;
        const {data,error} = await supabase.from('leads').insert({...leadData, subcategory: subcategory||''}).select().single();
        if(error) throw error;
            ...
            <div style={{...CC.card,padding:22}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                ...
                <div>
                  ...
                  <input className="inp" list="edit-cat-list" value={editLead.category||''} onChange={e=>setEditLead({...editLead,category:e.target.value})} placeholder="Skriv kategori..."/>
                  <datalist id="edit-cat-list">{categories.map(c=><option key={c} value={c}/>)}</datalist>
                </div>
                <div>
                  <label>Underkategori</label>
                  <input className="inp" value={editLead.subcategory||''} onChange={e=>setEditLead({...editLead,subcategory:e.target.value})} placeholder="Valgfri underkategori..."/>
                </div>
                <div>
                  <label>Land</label>
                  <input className="inp" list="edit-country-list" value={editLead.country||''} onChange={e=>setEditLead({...editLead,country:e.target.value})} placeholder="Land..."/>
                  <datalist id="edit-country-list">{countries.map(c=><option key={c} value={c}/>)}</datalist>
                </div>
          ...
          <div style={{padding:28,maxWidth:640}}>
            ...
            <div style={{...CC.card,padding:20,marginBottom:12}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}>
                {[['Email',sel.email],['Telefon',sel.phone],['By',sel.city],['Land',sel.country],['Kategori',sel.category],['Underkategori',sel.subcategory],['Kontaktperson',sel.contact_person]].map(([lb,v])=>(
                  ...
                  {filtered.map(lead=>(
                    <tr key={lead.id} className={bulk?'':'rh'} style={{borderBottom:'1px solid #0d1420',background:bulkSel.has(lead.id)?'#7c3aed10':'transparent',cursor:bulk?'default':'pointer'}}
                      ...
                      <td style={{padding:'10px 14px'}}>
                        ...
                        {lead.subcategory&&<div style={{fontSize:10,color:'#6b7280',marginTop:2}}>{lead.subcategory}</div>}
                      </td>
...
}function parseLine(line, cat, country, allCountries) {
  const p = line;
  if(!Array.isArray(p) || p.length === 0 || (p.length === 1 && !p[0])) return null;
  if(isHeader(p)) return null;
  ...
  if(isFixedFormat) {
    name = p[0].trim();
    category = p[1] ? p[1].trim() : cat;
    subcategory = p[2] ? p[2].trim() : '';
    land = detectCountry(p[3], countries, country);
    mail = (p[4]||'').replace(/\s/g,'');
    phone = p[5] ? p[5].trim() : '';
    city = p[6] ? p[6].trim() : '';
    const otrCols = [p[7], p[8], p[9]].filter(x => x && x.trim());
    for(const c of otrCols) {
      if(isSaleField(c)) { saleField = c; outreaches.push(...parseOtrField(c, true)); }
      else { outreaches.push(...parseOtrField(c, false)); }
    }
    if(p[10] && isSaleField(p[10])) saleField = p[10];
  } else {
    // Generic fallback
    name = p[0].trim();
    subcategory = '';
    const ei = findEmail(p);
    if(ei >= 0) {
      mail = p[ei].replace(/\s/g,'');
      land = detectCountry(p[2]||'', countries, country);
      const col1 = (p[1]||'').trim();
      const looksLikeType = /klub|skole|forening|center|butik|shop|board|surf|kajak|sejl|vinter|spejder|gym|børn|natur|havn|ski|kite|sup|wake|blokart|drager/i.test(col1);
      if(looksLikeType) { category = col1; }
      else { category = cat; city = col1; }
      const remCols = p.slice(ei+1);
      for(const c of remCols) {
  ...
  if(!name) return null;
  if(!mail) {
    const found = p.find(x => isEmail(x));
    if(found) mail = found.replace(/\s/g,'');
  }

  // Build display category: "Kategori (Underkategori)" or just "Kategori"
  const displayCategory = category && subcategory ? `${category} (${subcategory})` : (category || cat);

  const hasSale = !!saleField;
  let status = 'not_contacted';
  if(hasSale) status = 'won';
  else if(outreaches.filter(o=>o.note).length > 0) status = 'outreach_done';
...
function isEmail(s) {
  ...
  return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(s.replace(/\s/g,''));
}───────
const DEFAULT_CATEGORIES = [];
const DEFAULT_COUNTRIES = ['Danmark','Norge','Sverige'];
const STATUS_OPTIONS = [
