...
function parseCSVLine(line) {
  const t=[]; let c='',inQ=false,qc='';
  for(const ch of line){
    if(inQ){if(ch===qc)inQ=false;else c+=ch;}
    else if(ch==='"'||ch==="'"){inQ=true;qc=ch;}
    else if(ch===','||ch==='\t'||ch===';'){t.push(c.trim());c='';}
    else c+=ch;
  }
  t.push(c.trim()); return t;
}
function isHeader(p){const l=p.map(x=>x.toLowerCase());return l.some(x=>x==='navn'||x==='name'||x==='mail'||x==='email'||x==='land');}
function findEmail(p){return p.findIndex(x=>/^[^\s@,]+@[^\s@,]+\.[^\s@,]{2,}$/.test(x));}
function parseOtrRaw(raw){
  if(!raw||!raw.trim())return[];
  return raw.split(/[;|]+/).map(s=>s.trim()).filter(Boolean).map(part=>{
    const dm=part.match(/(\d{1,2}[\.\/-]\d{1,2}[\.\/-]\d{2,4})/);
    const bm=part.match(/^([A-Za-z\xC6\xE6\xD8\xF8\xC5\xE5\s]+?)[\s-]+\d/);
    const sm=part.match(/solgt[^)]*/i);
    return{date:dm?dm[1]:'',by:bm?bm[1].trim():'Ukendt',note:part,sale_info:sm?sm[0]:''};
  });
}
function detectSale(s){if(!s)return null;const m=s.match(/solgt\s+(\d+)\s*stk[^)]*/i);return m?m[0]:null;}
function parseLine(line,cat,country){
  ...
  let name='',city='',ctry='',email='',oRaw='',prod='',contact='',note='',phone='';
  if(ei>=0){
    email=p[ei];
    if(ei>=3){name=p[0]||'';city=p[1]||'';ctry=p[2]||country;}
    else if(ei===2){name=p[0]||'';city=p[1]||'';ctry=country;}
    else{name=p[0]||'';ctry=country;}
    oRaw=p[ei+1]||'';prod=p[ei+2]||'';contact=p[ei+3]||'';note=p[ei+4]||'';
  }else{name=p[0]||'';city=p[1]||'';ctry=p[2]||country;}
  if(!phone)phone=p.find(x=>x!==email&&/^\+?\d[\d\s-]{5,}$/.test(x))||'';
  const knownC=['Danmark','Sverige','Norge'];
  const cf=knownC.find(c=>(ctry||'').toLowerCase().startsWith(c.toLowerCase()))||country;
  const outreaches=parseOtrRaw(oRaw);
  const s1=detectSale(oRaw),s2=detectSale(note);
  let sale_info='',status='not_contacted';
  if(s1||s2){
    status='won';
    const pp=[];if(s1)pp.push(s1);if(prod)pp.push('Produkt: '+prod);if(contact)pp.push('Kontakt: '+contact);
    sale_info=pp.join(' | ');
  }else if(outreaches.length>0)status='outreach_done';
  const np=[];
  if(!sale_info&&contact)np.push('Kontakt: '+contact);
  if(!sale_info&&prod)np.push('Produkt: '+prod);
  if(note)np.push(note);if(city)np.push('By: '+city);
  if(!name&&!email)return null;
  return{name,category:cat,country:cf,email,phone,city,status,_outreaches:outreaches,notes:np.join('\n'),sale_info,contact_person:contact,product:prod};
}
...
export default function CRMApp() {
  ...
  const loadLeads = async () => {
    ...
    try {
      ...
      const { data: outreachData, error: oError } = await supabase.from('outreaches').select('*').order('date', { ascending: true });
      ...
      setLeads((leadsData||[]).map(l => ({...l, outreaches: oByLead[l.id]||[]})));
    } catch(e) { msg('Fejl ved indlæsning: '+e.message, 'err'); }
    setLoading(false);
  };
  ...
  const stats={ total:leads.length, won:leads.filter(l=>l.status==='won').length, out:leads.filter(l=>l.status==='outreach_done'||l.status==='in_dialogue').length, nc:leads.filter(l=>l.status==='not_contacted').length };
  const runP = txt => txt.trim().split('\n').filter(Boolean).map(l=>parseLine(l,iCat,iCountry)).filter(Boolean);
  ...
  const saveLead = async () => {
    ...
    try {
      ...
      const payload = { name:editLead.name, category:editLead.category, country:editLead.country, email:editLead.email, phone:editLead.phone, city:editLead.city, status:editLead.status, notes:editLead.notes, sale_info:editLead.sale_info, contact_person:editLead.contact_person, product:editLead.product };
      if(isExisting){
  ...
  const addOtr = async lead => {
    ...
    try {
      const {data,error} = await supabase.from('outreaches').insert({ lead_id:lead.id, date:newOtr.date, by:newOtr.by, note:newOtr.note, sale_info:newOtr.sale_info||'' }).select().single();
      if(error) throw error;
      if(status!==lead.status){
        ...
        setLeads(leads.map(l=>l.id===lead.id?updated:l)); setSel(updated);
      } else {
  ...
  const saveEditOtr = async lead => {
    try {
      const {error} = await supabase.from('outreaches').update({ date:editOtr.date, by:editOtr.by, note:editOtr.note, sale_info:editOtr.sale_info||'' }).eq('id',editOtrId);
      if(error) throw error;
      const updated={...lead,outreaches:lead.outreaches.map(o=>o.id===editOtrId?{...o,...editOtr}:o)};
      setLeads(leads.map(l=>l.id===lead.id?updated:l)); setSel(updated); setEditOtrId(null); setEditOtr(null); msg('Opdateret');
    } catch(e) { msg('Fejl: '+e.message,'err'); }
  };
  ...
  const updSt = async (lead,status) => {
    try {
      ...
      setLeads(leads.map(l=>l.id===lead.id?updated:l)); setSel(updated); msg('Status opdateret');
    } catch(e) { msg('Fejl: '+e.message,'err'); }
  };
  ...
  const applyBulk = async () => {
    ...
    try {
      ...
      if(bulkDate){ const rows=ids.map(id=>({lead_id:id,date:bulkDate,by:bulkBy,note:bulkNote,sale_info:''})); await supabase.from('outreaches').insert(rows); }
      await loadLeads(); msg(bulkSel.size+' leads opdateret'); setBulkSel(new Set()); setBulk(false);
    } catch(e) { msg('Fejl: '+e.message,'err'); }
    setSaving(false);
  };
  ...
  const importLeads = async () => {
    ...
    try {
      for(const lead of iPrev){
        ...
        if(error) throw error;
        if(_outreaches&&_outreaches.length>0){ const rows=_outreaches.map(o=>({lead_id:data.id,...o})); await supabase.from('outreaches').insert(rows); }
      }
      await loadLeads(); setIText(''); setIPrev([]); setView('list'); msg(iPrev.length+' leads importeret');
    } catch(e) { msg('Fejl: '+e.message,'err'); }
    setSaving(false);
  };
  ...
  const connectShopify = async () => {
    ...
    try {
      ...
      const d=await r.json(); setShopOrders(d.orders||[]); setShopOK(true);
      msg('Tilsluttet - '+(d.orders||[]).length+' ordrer hentet'); setView('dashboard');
  ...
  const refreshShop = async () => {
    ...
    try {
      ...
      const d=await r.json(); setShopOrders(d.orders||[]); msg('Shopify opdateret');
    } catch(e) { msg('Fejl: '+e.message,'err'); }
    setShopLoading(false);
  };
  ...
  const CC={ card:{background:'#111827',border:'1px solid #1f2937',borderRadius:14}, inner:{background:'#0d1420',border:'1px solid #1a2332',borderRadius:10} };
  const NAV=[{id:'dashboard',label:'Dashboard'},{id:'list',label:'Leads'},{id:'import',label:'Importér'},{id:'shopify_settings',label:'Shopify'}];
  ...
  return(
    <div style={{display:'flex',minHeight:'100vh',background:'#0a0f1e',color:'#e2e8f0',fontFamily:'system-ui,sans-serif'}}>
      ...
      <div style={{width:160,background:'#080d18',borderRight:'1px solid #0f172a',display:'flex',flexDirection:'column',padding:'20px 10px',gap:2,position:'sticky',top:0,height:'100vh',flexShrink:0}}>
        ...
        {NAV.map(n=>(<button key={n.id} className={'navbtn'+(view===n.id?' active':'')} onClick={()=>{setBulk(false);setView(n.id);}}>{n.label}</button>))}
      ...
      <div style={{flex:1,overflow:'auto',minWidth:0}}>
        ...
        {view==='dashboard'&&(
          <div style={{padding:28}}>
            ...
            {shopOK&&(
              <>
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:14}}>
                  {[{l:'Total omsætning',v:totalRev.toLocaleString('da-DK',{maximumFractionDigits:0})+' kr',c:'#22c55e',sub:(growth>0?'+':'')+growth.toFixed(1)+'% vs forrige måned'},{l:'Betalte ordrer',v:paid.length,c:'#0ea5e9',sub:'Alle tider'},{l:'Denne måned',v:revThis.toLocaleString('da-DK',{maximumFractionDigits:0})+' kr',c:'#f59e0b',sub:new Date().toLocaleString('da-DK',{month:'long',year:'numeric'})}].map(s=>(
                    <div key={s.l} style={{...CC.card,padding:'18px 20px'}}>
                      <div style={{fontSize:10,color:s.c,fontWeight:700,textTransform:'uppercase',letterSpacing:0.5,marginBottom:6}}>{s.l}</div>
            ...
            {!shopOK&&(<div style={{...CC.card,padding:24,textAlign:'center',marginBottom:14}}><div style={{fontSize:14,fontWeight:600,marginBottom:6}}>Tilslut Shopify for omsætnings-dashboard</div><div style={{color:'#4b5563',fontSize:13,marginBottom:14}}>Se ordrer, omsætning og top-produkter direkte her</div><button className="btn btn-p" onClick={()=>setView('shopify_settings')}>Tilslut Shopify</button></div>)}
            <div style={{...CC.card,padding:20}}>
              ...
              <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
                {CATEGORIES.map(cat=>{const cnt=leads.filter(l=>l.category===cat).length;const wonC=leads.filter(l=>l.category===cat&&l.status==='won').length;if(!cnt)return null;return(<div key={cat} style={{...CC.inner,padding:'10px 14px',cursor:'pointer'}} onClick={()=>{setFCat(cat);setView('list');}}><div style={{fontSize:13,fontWeight:600,marginBottom:2}}>{cat}</div><div style={{fontSize:11,color:'#4b5563'}}>{cnt} leads · <span style={{color:'#22c55e'}}>{wonC} solgt</span></div></div>);})}
              </div>
            </div>
          </div>
        )}
        ...
        {view==='shopify_settings'&&(
          <div style={{padding:28,maxWidth:520}}>
            ...
            <div style={{...CC.card,padding:22,marginBottom:14}}>
              <div style={{marginBottom:12}}><label>Butiks-URL</label><input className="inp" value={shopDomain} onChange={e=>setShopDomain(e.target.value)} placeholder="surfmore.myshopify.com"/></div>
              <div style={{marginBottom:16}}><label>Admin API Access Token</label><input className="inp" type="password" value={shopToken} onChange={e=>setShopToken(e.target.value)} placeholder="shpat_xxxxxxxx"/></div>
        ...
        {view==='import'&&(
          <div style={{padding:28,maxWidth:760}}>
            ...
            <div style={{...CC.card,padding:22}}>
              <div style={{background:'#080d18',border:'1px solid #1a2332',borderRadius:8,padding:'10px 14px',marginBottom:14,fontSize:12,color:'#4b5563',fontFamily:'monospace'}}>Format: Navn, By, Land, Mail, B2B Outreach, Produkt, Hvem?, Notat</div>
              ...
              <div style={{marginBottom:14}}><label>Eller indsæt data direkte</label><textarea className="inp" rows={6} value={iText} onChange={e=>{setIText(e.target.value);setIPrev(runP(e.target.value));}} style={{resize:'vertical',fontFamily:'monospace',fontSize:12}} placeholder="Navn,By,Land,Mail,B2B Outreach,Produkt,Hvem?,Notat"/></div>
              {iPrev.length>0&&(
                <div>
                  ...
                  <div style={{maxHeight:240,overflow:'auto',marginBottom:14,border:'1px solid #1f2937',borderRadius:8}}>
                    <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
                      <thead><tr style={{background:'#080d18',position:'sticky',top:0}}>{['Navn','Email','By','Status','Outreach','Salg'].map(h=><th key={h} style={{padding:'7px 10px',textAlign:'left',color:'#4b5563',fontWeight:700,fontSize:10,textTransform:'uppercase',borderBottom:'1px solid #1f2937'}}>{h}</th>)}</tr></thead>
                      <tbody>{iPrev.map((l,i)=>(<tr key={i} style={{borderBottom:'1px solid #0d1420',background:i%2?'#ffffff03':'transparent'}}><td style={{padding:'5px 10px',fontWeight:600,maxWidth:140,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{l.name}</td><td style={{padding:'5px 10px',color:l.email?'#38bdf8':'#ef4444'}}>{l.email||'mangler'}</td><td style={{padding:'5px 10px',color:'#4b5563'}}>{l.city||'—'}</td><td style={{padding:'5px 10px'}}><StatusBadge value={l.status}/></td><td style={{padding:'5px 10px',color:'#f59e0b'}}>{l._outreaches?.length>0?l._outreaches.length+'x':'—'}</td><td style={{padding:'5px 10px',color:l.sale_info?'#22c55e':'#4b5563'}}>{l.sale_info||l.product||'—'}</td></tr>))}</tbody>
                    </table>
                  </div>
          ...
          <div style={{padding:28,maxWidth:640}}>
            ...
            <div style={{...CC.card,padding:22}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                {[['Navn *','name','text'],['Email','email','email'],['Telefon','phone','text'],['By','city','text']].map(([lb,k,t])=>(<div key={k}><label>{lb}</label><input className="inp" type={t} value={editLead[k]||''} onChange={e=>setEditLead({...editLead,[k]:e.target.value})}/></div>))}
          ...
          <div style={{padding:28,maxWidth:640}}>
            ...
            <div style={{...CC.card,padding:20,marginBottom:12}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}>
                {[['Email',sel.email],['Telefon',sel.phone],['By',sel.city],['Land',sel.country],['Kategori',sel.category],['Kontaktperson',sel.contact_person]].map(([lb,v])=>(<div key={lb}><div style={{fontSize:11,color:'#4b5563',marginBottom:2}}>{lb}</div><div style={{fontSize:14}}>{v||'—'}</div></div>))}
              </div>
            ...
            <div style={{...CC.card,padding:18,marginBottom:12}}>
              ...
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                {STATUS_OPTIONS.map(s=>(<button key={s.value} onClick={()=>updSt(sel,s.value)} style={{cursor:'pointer',border:'1px solid '+(sel.status===s.value?s.color:'#1f2937'),background:sel.status===s.value?s.color+'22':'transparent',color:sel.status===s.value?s.color:'#4b5563',borderRadius:7,padding:'6px 13px',fontSize:12,fontWeight:600,fontFamily:'inherit'}}>{s.label}</button>))}
              </div>
            </div>
            <div style={{...CC.card,padding:20}}>
            ...
            <div style={{...CC.card,overflow:'auto'}}>
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
                ...
                <tbody>
                  ...
                  {filtered.map(lead=>(
                    <tr key={lead.id} className={bulk?'':'rh'} style={{borderBottom:'1px solid #0d1420',background:bulkSel.has(lead.id)?'#7c3aed10':'transparent',cursor:bulk?'default':'pointer'}} onClick={()=>{if(!bulk){setSel(lead);setView('detail');}}}>
                      {bulk&&<td style={{padding:'10px 8px 10px 14px'}} onClick={e=>{e.stopPropagation();const n=new Set(bulkSel);n.has(lead.id)?n.delete(lead.id):n.add(lead.id);setBulkSel(n);}}><input type="checkbox" checked={bulkSel.has(lead.id)} readOnly style={{width:16,height:16,cursor:'pointer',accentColor:'#7c3aed'}}/></td>}
